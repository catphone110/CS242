...
public class FlyingBishop extends Piece {
    /**
     * Move like a queen but restricted to a distance of two squares
     * */
    public FlyingBishop(int color, Position position, Board board){
    ...
    public int judgeMove(int i, int j){
        ...
        if (Math.abs(this.getRow() - i)% 2!=0 || Math.abs(this.getCol() - j)% 2!=0)
        ...
        int step_row = (i - this.getRow()) > 0? 2: -2;
        int step_col = (j - this.getCol()) > 0? 2: -2;
    ...
    public Deque<Position> potentialMoves(){
        ...
        int col = this.getCol();
        // upper left
        for (int r_step = 2, c_step = -2; row + r_step < 8 && col + c_step > -1; r_step ++, c_step --){
        ...
        // upper right
        for (int r_step = 2, c_step = 2; row + r_step < 8 && col + c_step < 8; r_step ++, c_step ++){
            if (judgeMove(row+r_step, col+c_step)>=0){
                deque.add(new Position(row+r_step, col+c_step));
                if (this.getBoard().chessPieces[row+r_step][col+c_step]!=null) {break;}
            }
        ...
        // lower left
        for (int r_step = -2, c_step = -2; row + r_step > -1 && col + c_step > -1; r_step --, c_step --){
            if (judgeMove(row+r_step, col+c_step)>=0){
                deque.add(new Position(row+r_step, col+c_step));
                if (this.getBoard().chessPieces[row+r_step][col+c_step]!=null) {break;}
            }
        ...
        // lower right
        for (int r_step = -2, c_step = 2; row + r_step > -1 && col + c_step < 8; r_step --, c_step ++){
            if (judgeMove(row+r_step, col+c_step)>=0){
                deque.add(new Position(row+r_step, col+c_step));
                if (this.getBoard().chessPieces[row+r_step][col+c_step]!=null) {break;}
            }
